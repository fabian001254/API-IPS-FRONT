...
export async function paciente(address) {
    ...
    const respuesta = await fetch(address + formType, {
      ...
      "body": JSON.stringify({ cedula:cedula, nombre:nombre, apellido:apellido, fechaNacimiento:fecha, telefono:telefono})
    }); 
  ...
  export async function tablaPacientes(address){
    ...
    for (const pacientes of datos.pacientes){
        const fila = tabla.insertRow();
        fila.className = "text-center"; 
        ...
        const fecha = pacientes.fechaNacimiento.split("T")
